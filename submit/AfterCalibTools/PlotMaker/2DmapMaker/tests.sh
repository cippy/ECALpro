...
wwwBasePath="/afs/cern.ch/user/m/mciprian/www/pi0calib/ICplot/tests/"   # base directory where plots are stored (other directories are created inside this path) 

# list of source files to compile (except main) without extension (which is supposed to be .C in the following)
sourceFileList=(calibAnaEcal_base calibAnaEcal calibAnaEcalEB calibAnaEcalEE testsInEB)  
mainSourceFile="tests"
